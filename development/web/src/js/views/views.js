...
window.MyLocationDetailView = window.LocationDetailView.extend({
	initialize: function() {
		...
		_(this).bindAll("render", "renderPhoto");
		this.model.bind('change', this.model.getPhoto());
		window.loc = this.model;
	},
...
window.MyPlacesView = Backbone.View.extend({
	...
	render: function() { 
		forge.tools.getURL("img/title.jpg", function (myUrl) {
			alert(myUrl);
			window.titleUrl = myUrl;
			}, function(e){
			console.log(e);
		});
		
		forge.topbar.setTitleImage(window.titleUrl, function(e){
		...
		forge.topbar.setTint([234, 234, 234, 255]);
		forge.topbar.removeButtons();
		forge.topbar.addButton({
			text: "Add",
			position: "right"
		}, function() {
			window.App.navigate("#add", {
				trigger: true
			});
		});
	...
	renderStandardListItem: function(place) {
		window.place = place;
		return "<li data-icon='false' class='non_theme placeListItem'><a href='#mylocation/" + place.get("id") + "' data-transition='slide'><span class='listingName'>" + place.get("name") + "</span><span class='listingDetail'>" + place.get("location")["city"] + " ," + place.get("location")["state"] + "</span><span class='listingDetail'>" + place.get("category") + "</span><div class='dblArrow'><img src='img/dbl_arrow.jpg' alt='View this place' /></div><div class='recommendedBy'><span class='recommendedByName'>Recommended by " + place.get("referrer") + "</span></div></a></li>";
	},

	renderNearDistanceListItem: function(place) {
		var distance = Math.round(place.get("distance") * 10) / 10;
		return "<li data-icon='false' class='placeListItem'><a href='#mylocation/" + place.get("id") + "' data-transition='slide'><span class='distance'>" + distance + " mi</span><span class='listingName'>" + place.get("name") + "</span><span class='listingDetail'>" + place.get("category") + "</span><div class='dblArrow'><img src='img/dbl_arrow.jpg' alt='View this place' /></div><div class='recommendedBy'><span class='recommendedByName'>Recommended by "+place.get("referrer")+"</span></div></a></li>";
	},
